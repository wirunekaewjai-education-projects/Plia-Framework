...
public class MainMenu extends Scene implements OnTouchListener
{
	private Game game;
	...
	public void onTouch(Button btn, int action, float x, float y)
	{
		if(action == TouchEvent.ACTION_UP)
		{
			...
			else
			{
				...
				{
					RaceScene raceScene = ((MainGame)game).getRaceScene();
					if(raceScene != game.getScene())
					{
						game.setScene(raceScene);
					}
				}
